...
protected:	// types
		/// auxiliary map
	typedef std::map<const TDepSetElement*,TDepSetElement*> CacheMap;
		/// map RW iterator
...
public:		// interface
	...
	TDepSetElement* get ( TDepSetElement* tail )
	{
		...
		TDepSetElement* concat = new TDepSetElement ( Manager, Level, tail );
		Map[tail] = concat;
		return concat;
	}
}; // TDepSetCache
...
public:		// interface
	...
	TDepSetElement* get ( unsigned int n, TDepSetElement* tail = NULL ) const { return Base[n]->get(tail); }
